...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
    class Synthesis_Ops_Commands extends WP_CLI_Command {
        ...
        function w3tc( $args, $assoc_args ) {
            ...
            if ( isset( $assoc_args['check'] ) ) {
                ...
                if ( isset( $assoc_args['preserve-cdn'] ) ) {
                    ...
                    $cdn_url = "http://tiagohillebrandt.eti.br/w3tc/" . $w3tc_version . "/cdn_settings";
        ...
        function flushtransients( $args, $assoc_args ) {
            ...
            if ( strlen( $url ) == 0 || !is_dir( "/var/www/" . $url ) ) {
                WP_CLI::error( "Unable to find site root. Skipping." );
                return;
            }
            
            if ( !is_writable( "/var/www/" . $url ) ) {
                WP_CLI::error( "Site root is not writable. Please check the permissions. Skipping." );
                return;
            } else {
                $backup_file = "." . $url . ".options.sql";
                $backup_file = preg_replace( "/\//", "_", $backup_file );

                WP_CLI::launch( "mysqldump --add-drop-table -u " . DB_USER . " --password=" . DB_PASSWORD . " " . DB_NAME . " $wpdb->options > /var/www/" . $url . "/" . $backup_file );
                WP_CLI::line( "Options table backup: /var/www/" . $url . "/" . $backup_file );
